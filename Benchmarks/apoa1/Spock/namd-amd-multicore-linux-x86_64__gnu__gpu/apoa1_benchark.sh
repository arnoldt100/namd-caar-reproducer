...
function declare_global_variables {
    ...
    if [[ ! -v NCP_APOA1_NAMD2_BINARY ]];then
        message="A FATAL ERROR HAS OCCURRED!\n"
        message+="\tThe environmental variable NCP_APOA1_NAMD2_BINARY is not set.\n"
        message+="\tPlease read the README.md for this benchmark to correct."
        error_exit "${message}"
    fi
    ...
    declare -gr NAMD2_BINARY="${NCP_APOA1_NAMD2_BINARY}"
    declare -gr NAMD3_BINARY="${NCP_APOA1_NAMD3_BINARY}"
...
function perform_benchmark {
    ...
    local -r my_batch_template_file=${3}
    local -r my_apoa1_benchmarks_tag=${4}
...
function main () {
    ...
    for apoa1_btag in "${my_apoa1_benchmarks_tags[@]}";do
        perform_benchmark ${SCRATCH_DIR} ${RESULTS_DIR} ${benchmark_file} ${apoa1_btag}
    done
}
