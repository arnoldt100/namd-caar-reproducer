...
function build_namd_multicore_linux_x86_64__gnu__cpu {
    ...
    ${config_command}
    echo "The NAMD config command: ${config_command}" 
    if [[ $? != 0 ]];then
...
function build_namd_amd_multicore_linux_x86_64__gnu__gpu {
    ...
    make -j ${nm_make_threads}
    if [[ $? != 0 ]];then
        local -r make_error_message="The make command of build_namd_amd_multicore_linux_x86_64__gnu__gpu failed."
    fi
...
#-----------------------------------------------------
# Function:                                          -
#    build_namd_amd_ofi_linux_x86_64_slurmpmi2__gnu__cpu -
#                                                    -
# Synopsis:                                          -
#   Builds the namd from the AMD rpository for       -
#   for multinode version.                           -
#                                                    -
# Positional parameters:                             -
#                                                    -
#-----------------------------------------------------
function build_namd_amd_ofi_linux_x86_64_slurmpmi2__gnu__cpu () {
    local -r machine_name="${MACHINE_NAME}"
    local -r namd_arch="${NAMD_ARCH}"
    local -r charm_arch="${CHARMARCH}"
    local -r prefix="${NAMD_PREFIX}"
    ...
    ${config_command}
    if [[ $? != 0 ]];then
        local -r config_error_message="The configure command of build_namd_amd_ofi_linux_x86_64_slurmpmi2__gnu__cpu failed."
        error_exit "${config_error_message}"
    fi
    ...
    make -j ${nm_make_threads}
    if [[ $? != 0 ]];then
        local -r make_error_message="The make command of build_namd_amd_ofi_linux_x86_64_slurmpmi2__gnu__cpu failed."
    fi
    ...
    case ${ncp_target_machine} in
        "Spock" )
            case ${ncp_target_build} in
                ...
                "namd-amd-ofi-linux-x86_64_slurmpmi2__gnu__cpu" )
                    build_namd_amd_ofi_linux_x86_64_slurmpmi2__gnu__cpu;;
